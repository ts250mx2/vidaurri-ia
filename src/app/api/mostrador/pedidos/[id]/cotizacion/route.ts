...
// pedido que el mostrador ya aceptó (confirmado, listo o entregado); si ya está
// insertada no se duplica: sincronizarCotizacionPos la deja como está.
...
export async function POST(request: Request, contexto: Contexto) {
  ...
  try {
    ...
    if (!puedeCotizarEnPos(pedido.estatus)) {
      return respuestaError("Solo se cotiza en el POS un pedido confirmado, listo o entregado", 409);
    }
